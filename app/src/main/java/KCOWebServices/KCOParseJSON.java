...
public class KCOParseJSON {
    ...
    public JSONObject getJSONFromUrl(String url, List<NameValuePair> params,int type) {
      ...
      if(type==0){
        ...
        try {
          ...
          // hacemos una peticion de tipo HTTP GET
          try {
              // cliente por defecto --> defaultHttpClient
              DefaultHttpClient httpClient = new DefaultHttpClient();
              String paramsGet = URLEncodedUtils.format(params, "UTF-8");
              HttpGet httpGet = new HttpGet(url+"?"+paramsGet);
